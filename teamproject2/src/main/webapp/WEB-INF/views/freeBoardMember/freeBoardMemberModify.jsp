...
function returnFreeBoardView(){
	location.href = "freeBoardMemberView?fbm_num=${param.fbm_num}&page=${param.page }&searchCategory=${param.searchCategory}&sportsCategory=${param.sportsCategory }&searchWord=${param.searchWord}";
}
		 ...
		 -->		<div id="body_list">
					<article >
						<div id="write_div">
						  ...
						  <form class="" action="freeBoardMemberModify" method="post" id = "modifyForm" enctype="multipart/form-data">
						  <input type = "hidden" name = "fbm_num" value = "${param.fbm_num }">
							  <div  id="write_category" class="write_class">
							  <label for="input_title" class="write_label">종목</label><br>
								  <select name = "fbm_category" id = "fbm_category">
								  	<option value = "축구" <c:if test="${map.freeBoardMemberVo.fbm_category == '축구' }">selected</c:if>>축구</option>
								  	<option value = "야구" <c:if test="${map.freeBoardMemberVo.fbm_category == '야구' }">selected</c:if>>야구</option>
								  	<option value = "배드민턴" <c:if test="${map.freeBoardMemberVo.fbm_category == '배드민턴' }">selected</c:if>>배드민턴</option>
								  	<option value = "테니스" <c:if test="${map.freeBoardMemberVo.fbm_category == '테니스' }">selected</c:if>>테니스</option>
								  </select>
							  </div>
							  <div  id="write_name" class="write_class">
							  <label for="input_title" class="write_label">제목</label><br>
							  <input type="text" id="input_title" class="input_class" name="fbm_title" value="${map.freeBoardMemberVo.fbm_title}" >
							  </div>
				  
							  <div class="write_class">
								<label for="input_name" class="write_label">작성자</label><br>
								<input type="text" id="input_name" class="input_class" name="fbm_nickname" value="${map.freeBoardMemberVo.m_nickname }" disabled="disabled">
							  </div>
				  
							  <div class="write_class" id="content_div">
								<label for="fbm_content" class="write_label">내용</label><br>
								<textarea name="fbm_content" cols="80" id="input_content" placeholder="" >${map.freeBoardMemberVo.fbm_content }</textarea>
							  </div>
