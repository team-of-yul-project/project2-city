...
function write_btn(){
	  ...
	  location.href="eventBoardWrite?page=${map.page }&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}";
	}
</script>
<style type="text/css">
.list_title_a{
...
<body>
					...
					<jsp:include page="../include/nav.jsp"/>
							...
							<div id="body_header_div">
								<div id="body_header_div1">
									<form action="eventBoardList" method="get">
					...
					<table id="list_table">
						...
						<c:forEach items="${map.list }" var = "eventBoardVo">
							<tr class="list_tr_td">
							  ...
							  <a class = "list_title_a"href="eventBoardView?eb_num=${eventBoardVo.eb_num }&page=${map.page }&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}">${eventBoardVo.eb_title }</a>
							  </td>
							  <td>관리자</td>
							  <td>${eventBoardVo.eb_date }</td>
    ...
    <c:if test="${map.page != 1}">
    	<a href="eventBoardList?page=1&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}"> <li class="first"></li></a>
    </c:if>
    
    
    <!-- 이전페이지 버튼 1보다크면-->
    <c:if test="${map.page > 1}">
    	<a href="eventBoardList?page=${map.page-1 }&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}"> <li class="prev"></li></a>
    </c:if>
     ...
     <c:forEach var="nowPage" begin="${map.startPage}" end="${map.endPage}">
     	...
     	<c:if test="${map.page != nowPage}">
    		<a href="eventBoardList?page=${nowPage }&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}"> 
      ...
      <c:if test="${map.page < map.endPage}">
     	 <a href="eventBoardList?page=${map.page+1 }&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}"> <li class="next"></li></a>
      </c:if>
      
      
      <c:if test="${map.page !=  map.maxPage}">
      	<a href="eventBoardList?page=${map.maxPage }&searchCategory=${map.searchCategory}&sportsCategory=${map.sportsCategory }&searchWord=${map.searchWord}"> <li class="last"></li></a>
      </c:if>
